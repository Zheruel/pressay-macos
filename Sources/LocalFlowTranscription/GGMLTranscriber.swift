...
public actor GGMLTranscriber: SpeechTranscriber {
    ...
    private func load() async throws {
        guard let download = asrModel.ggufDownload else {
            throw LocalFlowError.modelUnavailable("\(asrModel.displayName) has no GGUF artifact")
        }
        let directory = FileManager.default
            .urls(for: .applicationSupportDirectory, in: .userDomainMask)[0]
            .appending(path: "LocalFlow/Models/GGUF", directoryHint: .isDirectory)
        ...
        if !FileManager.default.fileExists(atPath: destination.path) {
            ...
            guard let http = response as? HTTPURLResponse, http.statusCode == 200 else {
                ...
                throw LocalFlowError.modelUnavailable("Model download failed for \(asrModel.displayName)")
            }
    ...
    public func transcribe(_ clip: AudioClip) async throws -> ASRTranscript {
        ...
        guard let session else {
            throw LocalFlowError.modelUnavailable("\(asrModel.displayName) is not loaded")
        }
