...
// LocalFlowBench — dev-only evaluation harness.
...
struct Bench {
    ...
    static func runKimiPolish(entries: [ManifestEntry], options: [String: String], outDir: URL) async throws {
        ...
        print("kimi-polish: \(clips.count) clips × \(templates.count) templates × models \(models)")
        ...
        for model in models {
            for (name, template) in templates.sorted(by: { $0.key < $1.key }) {
                let label = models.count > 1 ? "\(model)_\(name)" : name
                for clip in clips {
                    ...
                    do {
                        let output = try await client.polish(
                            cleaned, template: template, model: model, apiKey: apiKey, timeout: 120
                        )
                        let latency = started.duration(to: .now).seconds
                        try? output.write(
