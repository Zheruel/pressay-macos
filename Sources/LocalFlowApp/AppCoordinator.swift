...
final class AppCoordinator: ObservableObject, HoldHotkeyDelegate {
    ...
    func restartHotkey() {
        ...
        do {
            ...
            if settings.polishHoldKey == settings.holdKey {
                polishHotkey.stop()
                lastError = "Prompt polish is off: both features use the same key"
            } else {
        ...
        guard stateMachine.begin() else {
            if stateMachine.phase == .processing {
                overlay.showError(currentMode == .promptPolish
                    ? "Still polishing the previous dictation"
                ...
                do {
                    guard let apiKey = KimiAPIKeyStore.read(), !apiKey.isEmpty else {
                        throw LocalFlowError.modelUnavailable(
                            "Add a Kimi API key in Settings to use prompt polish"
                        )
                    }
                    let candidate = try await Timeout.run(for: .seconds(45)) { [kimiPolisher] in
                        try await kimiPolisher.polish(deterministic, apiKey: apiKey)
                    }
                    ...
                    fail(error, message: "Polish failed — transcript saved to History")
                    return
                }
            }
    ...
    private func captureFailedMidDictation(_ message: String) {
        guard stateMachine.phase == .recording else { return }
        fail(LocalFlowError.microphoneUnavailable, message: message)
    }
