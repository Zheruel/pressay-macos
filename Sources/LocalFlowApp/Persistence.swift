...
final class HistoryStore: ObservableObject {
    ...
    func applyRetention(now: Date = .now) throws {
        ...
        try removeExpiredOrphanedAudio(
            referencedPaths: Set(allRecords.compactMap(\.audioPath)),
            now: now
        )
        try context.save()
        try refresh()
    ...
    private func removeExpiredOrphanedAudio(referencedPaths: Set<String>, now: Date) throws {
...
enum AudioFileStore {
    static var directory: URL {
        ...
        return base.appending(path: "LocalFlow/Audio", directoryHint: .isDirectory)
    }
